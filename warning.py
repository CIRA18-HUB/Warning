...
def create_age_distribution_chart(age_data, max_display=10, show_all=False):
    """创建库龄分布条形图 - 按风险程度排序并限制显示数量"""
    ...
    # 添加排序列
    sorted_age_data = age_data.copy()
    sorted_age_data['风险排序'] = sorted_age_data['风险程度'].map(risk_order)

    # 按风险程度和库龄排序
    sorted_age_data = sorted_age_data.sort_values(['风险排序', '库龄'], ascending=[True, False])

    # 确定是否限制显示
    if not show_all:
        display_data = sorted_age_data.head(max_display)
    else:
        display_data = sorted_age_data

    # 创建标签 - 使用简化产品名称
    if '简化产品名称' in display_data.columns:
        labels = [f"{name} ({date})" for name, date in
                  zip(display_data['简化产品名称'], display_data['批次日期'].astype(str))]
    else:
        labels = [f"{code} ({date})" for code, date in
                  zip(display_data['物料'], display_data['批次日期'].astype(str))]
    ...
    fig = px.bar(
        ...
        y=labels,
        orientation='h',
        title='批次库龄分布' + (f' (显示前{max_display}个最高风险批次)' if not show_all else ''),
        color='风险程度',
        color_discrete_map=color_map,
        text='库龄',
        hover_data=['物料', '描述', '批次日期', '批次库存', '风险程度', '预计清库天数', '批次价值', '责任人']
    )
    ...
    fig.update_layout(
        xaxis_title='库龄 (天)',
        yaxis_title='批次',
        height=max(500, min(len(display_data) * 30, 800)),  # 动态调整高度，但限制最大高度
...
def create_forecast_bias_chart(bias_data, max_display=20):
    ...
    try:
                ...
                def extract_bias_value(bias_str):
                    try:
                        if isinstance(bias_str, str) and '%' in bias_str:
                            return float(str(bias_str).rstrip('%')) / 100
                        elif isinstance(bias_str, (int, float)):
                            return float(bias_str)
                        else:
                            return 0.0
                    except Exception as e:
                        print(f"无法转换预测偏差值 '{bias_str}': {str(e)}")
                        return 0.0
        ...
        # 确保我们有正负偏差值，如果都是正的或都是负的，检查转换逻辑
        if all(bias_data['预测偏差数值'] >= 0) or all(bias_data['预测偏差数值'] <= 0):
            print("警告：所有预测偏差都是同一方向，请检查数据计算逻辑")
        ...
        if not positive_bias.empty:
            fig.add_trace(go.Bar(
                y=[labels[i] for i, idx in enumerate(top_bias.index) if idx in positive_bias.index],
                ...
                hovertemplate='<b>%{y}</b><br>' +
                              ...
                              '描述: %{customdata[1]}<br>' +
                              '批次库存: %{customdata[3]:,.0f} 件<br>' +
                              '日均出货: %{customdata[4]:.2f} 件/天<br>' +
                              '预计清库天数: %{customdata[5]}<br>' +
                              '责任人: %{customdata[6]}<br>' +
                              '<extra>预测过高</extra>',
                customdata=[[
                    row['物料'],
                    row['描述'][:30] + '...' if isinstance(row['描述'], str) and len(row['描述']) > 30 else row['描述'],
                    row['批次日期'].strftime('%Y-%m-%d') if hasattr(row['批次日期'], 'strftime') else str(row['批次日期']),
                ...
                ] for _, row in positive_bias.iterrows()]
            ))
        ...
        if not negative_bias.empty:
            fig.add_trace(go.Bar(
                y=[labels[i] for i, idx in enumerate(top_bias.index) if idx in negative_bias.index],
                ...
                customdata=[[
                    ...
                    row['责任人'] if '责任人' in row else '',
                    row['责任区域'] if '责任区域' in row else ''
                ] for _, row in negative_bias.iterrows()]
            ))

        # 零偏差条形图（如果有的话）
        zero_bias = top_bias[top_bias['预测偏差数值'] == 0]
        if not zero_bias.empty:
            fig.add_trace(go.Bar(
                y=[labels[i] for i, idx in enumerate(top_bias.index) if idx in zero_bias.index],
                x=[0] * len(zero_bias),
                name='无偏差',
                orientation='h',
                ...
                customdata=[[
                    ...
                    row['责任人'] if '责任人' in row else '',
                    row['责任区域'] if '责任区域' in row else ''
                ] for _, row in zero_bias.iterrows()]
            ))
        ...
        fig.update_layout(
            ...
            plot_bgcolor='white',
            height=max(500, min(len(top_bias) * 30, 800)),  # 限制最大高度
            margin=dict(l=200, r=80, t=80, b=80)
        )
...
def create_forecast_accuracy_chart(bias_data):
    """创建预测准确度散点图 - 修复错误和增强悬停效果"""
    if bias_data is None or bias_data.empty:
        return go.Figure()

    # 筛选有效数据并确保必要列存在
    valid_data = bias_data.copy()
    if '预测偏差数值' not in valid_data.columns:
        # 如果没有预测偏差数值列，尝试创建
        if '预测偏差' in valid_data.columns:
            valid_data['预测偏差数值'] = valid_data['预测偏差'].apply(
                lambda x: float(str(x).rstrip('%')) / 100 if isinstance(x, str) and '%' in str(x) else
                (float(x) if isinstance(x, (int, float)) else 0)
            )
        else:
            # 如果没有必要的列，返回空图表
            fig = go.Figure()
            fig.update_layout(title="无法获取预测偏差数据，缺少必要的列")
            return fig

    # 确保所有必要的列都存在，否则创建默认值
    required_columns = ['批次价值', '预计清库天数', '风险程度']
    for col in required_columns:
        if col not in valid_data.columns:
            valid_data[col] = 1 if col == '批次价值' else ('中风险' if col == '风险程度' else 30)

    # 处理无穷大值
    valid_data['预计清库天数_显示'] = valid_data['预计清库天数'].apply(
        lambda x: 200 if x == float('inf') else float(x)
    )
    ...
    for _, row in valid_data.iterrows():
        hover_data = [
            ...
            row['描述'][:30] + '...' if isinstance(row['描述'], str) and len(row['描述']) > 30 else row[
                '描述'] if '描述' in row else '',
            float(row['批次库存']) if '批次库存' in row else 0,
            float(row['日均出货']) if '日均出货' in row else 0,
            '无法预测' if row['预计清库天数'] == float('inf') else f"{float(row['预计清库天数']):.1f}",
            row['责任人'] if '责任人' in row else '',
            row['责任区域'] if '责任区域' in row and pd.notna(row['责任区域']) else '无指定区域',
            row['简化产品名称'] if '简化产品名称' in row else row['物料'] if '物料' in row else ''
        ]
        customdata.append(hover_data)
    ...
    fig = px.scatter(
        ...
        color='风险程度',
        size='批次价值',
        hover_name='物料' if '物料' in valid_data.columns else None,
        title='预测准确度与清库关系分析',
        labels={
            '预测偏差数值': '预测偏差',
            '预计清库天数_显示': '预计清库天数 (天)'
    ...
    fig.update_traces(
        ...
        hovertemplate='<b>%{customdata[7]}</b><br>' +
                      '预测偏差: %{x:.1%}<br>' +
                      '预计清库天数: %{y:,.0f} 天<br>' +
                      '物料: %{customdata[0]}<br>' +
                      '描述: %{customdata[1]}<br>' +
    ...
    # 处理预计清库天数为无穷大的情况
    max_clearance = valid_data['预计清库天数_显示'].max() if not valid_data.empty else 200
...
def main():
    ...
    with tabs[1]:  # 库龄与清库分析
        ...
        if not age_data.empty:
            # 初始化会话状态变量用于控制是否显示全部批次
            if 'show_all_batches' not in st.session_state:
                st.session_state.show_all_batches = False

            # 创建批次库龄图
            age_fig = create_age_distribution_chart(age_data, max_display=10,
                                                    show_all=st.session_state.show_all_batches)
            st.plotly_chart(age_fig, use_container_width=True)

            # 添加"查看更多/收起"按钮
            total_batches = len(age_data)
            if total_batches > 10:
                button_label = "收起" if st.session_state.show_all_batches else f"查看全部 {total_batches} 个批次"
                if st.button(button_label):
                    st.session_state.show_all_batches = not st.session_state.show_all_batches
                    st.rerun()
