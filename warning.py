    ...
    def get_forecast_bias_data(self):
        ...
        try:
            ...
            if not bias_data.empty and '预测偏差数值' in bias_data.columns:
                try:
                    # 尝试使用key=abs函数
                    return bias_data.sort_values(by='预测偏差数值', key=lambda x: abs(x), ascending=False)
                except Exception as e:
                    print(f"使用key=abs排序时出错: {str(e)}，尝试替代方法")
                    # 如果key=abs失败，使用绝对值列排序
...
def create_age_distribution_chart(age_data, max_display=10):
    ...
    # 创建标签 - 使用简化产品名称
    if '简化产品名称' in top_age_data.columns:
        labels = [f"{name} ({date})" for name, date in
                  zip(top_age_data['简化产品名称'], top_age_data['批次日期'].astype(str))]
    else:
        labels = [f"{code} ({date})" for code, date in
                  zip(top_age_data['物料'], top_age_data['批次日期'].astype(str))]
...
def create_forecast_bias_chart(bias_data, max_display=10):
    ...
    try:
        ...
        fig.update_layout(
            ...
            # 增加高度以避免标签拥挤
            height=500 + max(0, (max_display - 7) * 30),
            # 增加左边距以显示较长的产品名称
            margin=dict(l=200, r=50, t=50, b=50)
        )
...
def create_fulfillment_rate_chart(batch_analysis):
    ...
    fig.update_layout(
        ...
        margin=dict(l=150, r=50, t=50, b=50)  # 增加左边距以显示责任人名称
    )
