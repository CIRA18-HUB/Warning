...
def setup_environment():
    ...
    is_cloud = 'STREAMLIT_SHARING' in os.environ or 'STREAMLIT_DEPLOYMENT' in os.environ

    if is_cloud:
        os.environ['STREAMLIT_DEPLOYMENT'] = 'cloud'
        # 显示当前环境信息
        print(f"检测到Streamlit Cloud环境")
        print(f"当前工作目录: {os.getcwd()}")
        print(f"目录内容: {os.listdir('.')}")
    else:
        os.environ['STREAMLIT_DEPLOYMENT'] = 'local'
...
def check_authentication():
    ...
    # 在Streamlit Cloud环境中，可以选择默认已认证
    # 这可以帮助您先测试其他功能是否正常，之后再恢复认证功能
...
class DataLoader:
    ...
    def __init__(self, data_dir="."):
        ...
        # 文件路径 - 直接使用文件名，不添加目录前缀
        self.inventory_file = "含批次库存0221(2).xlsx"
        self.shipping_file = "2409~250224出货数据.xlsx"
        self.forecast_file = "2409~2502人工预测.xlsx"
        self.price_file = "单价.xlsx"
    ...
    def load_all_data(self):
        ...
        return {
            ...
            "sales_person_region_mapping": self.sales_person_region_mapping,
            "product_responsibility": self.product_responsibility
        }
    ...
    def load_inventory_data(self):
        ...
        try:
            ...
            st.write(f"尝试加载的文件路径: {self.inventory_file}")
...
class AnalysisEngine:
    ...
    @st.cache_data
    def analyze_data(self, filtered_data=None):
        """
        分析数据并生成指标
        ...
        """
        # 使用原始数据或筛选后的数据
        batch_data_to_analyze = self.batch_data
        shipping_data_to_analyze = self.shipping_data
        ...
        for product_code in self.inventory_data['产品代码'].unique():
            ...
            if len(product_forecast) > 0:
                ...
                # 计算预测偏差
                forecast_bias = self.calculate_forecast_bias(forecast_quantity, actual_sales)
                ...
                for person, forecast_qty in person_forecast.items():
                    ...
                    # 计算个人预测偏差
                    person_bias[person] = self.calculate_forecast_bias(forecast_qty, person_recent_sales)
            else:
                # 无预测数据时
        ...
        for _, batch in batch_data_to_analyze.iterrows():
            ...
            # 生成责任分析摘要
            responsibility_summary = self.generate_responsibility_summary(responsibility_details)
        ...
        if not batch_analysis_df.empty:
            batch_analysis_df['风险排序'] = batch_analysis_df['风险程度'].map(self.risk_order)
        ...
        # 缓存分析结果
        self.batch_analysis = batch_analysis_df
    ...
    def get_forecast_bias_data(self):
        ...
        # 转换预测偏差为数值
        bias_data['预测偏差数值'] = bias_data['预测偏差'].apply(
        ...
        # 按预测偏差绝对值排序
        return bias_data.sort_values(by='预测偏差数值', key=abs, ascending=False)
...
def create_risk_distribution_chart(batch_analysis):
    ...
    # 对风险等级排序
    risk_counts['排序'] = risk_counts['风险等级'].map(risk_order)
    risk_counts = risk_counts.sort_values('排序')
...
def create_age_distribution_chart(age_data, max_display=10):
    """创建库龄分布条形图"""
    if age_data is None or age_data.empty:
        return go.Figure()

    # 选择前N个最高库龄批次
    top_age_data = age_data.sort_values('库龄', ascending=False).head(max_display)

    # 创建标签
    labels = [f"{code} ({date})" for code, date in zip(top_age_data['物料'], top_age_data['批次日期'].astype(str))]
    ...
    fig = px.bar(
        ...
        y=labels,
        orientation='h',
        title=f'批次库龄分布 (Top {max_display})',
        color='风险程度',
        color_discrete_map=color_map,
        text='库龄'
    )
    ...
    fig.update_layout(
        xaxis_title='库龄 (天)',
        yaxis_title='批次',
        height=500 + max(0, (max_display - 7) * 30)  # 根据显示批次数动态调整高度
    )
...
def create_clearance_chart(clearance_data, max_display=10):
    """创建清库预测图"""
    ...
    # 更新布局
    fig.update_layout(
        title=f'预计清库天数 vs. 库龄 (Top {max_display})',
        height=500 + max(0, (max_display - 7) * 30),  # 根据显示批次数动态调整高度
        legend=dict(
            orientation="h",
            yanchor="bottom",
...
def create_forecast_bias_chart(bias_data, max_display=10):
    """创建预测偏差图"""
    if bias_data is None or bias_data.empty:
        return go.Figure()

    # 选择预测偏差最显著的N个批次
    top_bias = bias_data.nlargest(max_display, '预测偏差数值', key=abs)

    # 创建标签
    labels = [f"{code} ({desc[:15]}...)" for code, desc in zip(top_bias['物料'], top_bias['描述'])]
    ...
    if not positive_bias.empty:
        fig.add_trace(go.Bar(
            y=labels[len(labels) - len(positive_bias):],
    ...
    if not negative_bias.empty:
        fig.add_trace(go.Bar(
            y=[labels[i] for i in range(len(negative_bias))],
...
def create_responsibility_region_chart(batch_analysis):
    ...
    # 计算区域责任分布
    region_counts = batch_analysis['责任区域'].value_counts().reset_index()
    region_counts.columns = ['责任区域', '批次数量']

    # 确保空区域显示为"无指定区域"
    region_counts['责任区域'] = region_counts['责任区域'].replace('', '无指定区域')
...
def create_responsibility_person_chart(batch_analysis, max_display=10):
    ...
    fig = px.bar(
        ...
        x='责任人',
        y='平均风险得分',
        title=f'责任人风险排名 (Top {max_display})',
        color='平均风险得分',
...
def create_fulfillment_rate_chart(batch_analysis):
    ...
    # 按责任人和区域分组计算平均履行率
    fulfillment_by_person_region = valid_data.groupby(['责任人', '责任区域'])['履行率'].mean().reset_index()
    ...
    fig = px.imshow(
        fulfillment_pivot,
        labels=dict(x="责任区域", y="责任人", color="履行率"),
...
def main():
    ...
    try:
        # 加载CSS样式
        load_css()
        logger.info("CSS样式加载完成")

        # 检查用户是否已登录
        if not check_authentication():
            logger.info("用户未认证，显示登录界面")
            return
        logger.info("用户认证通过")
        ...
        with st.sidebar:
            st.header("📂 数据控制")
            st.write("数据文件目录: `.`")  # 更新为根目录

            # 数据加载
            data_loader = DataLoader(data_dir=".")  # 使用根目录
        ...
        with tabs[0]:  # 风险预警概览
            ...
            with cols[0]:
                ...
                <b>图表解读：</b> 饼图展示了所有库存批次的风险分布情况，红色和橙色部分表示需要优先关注的高风险批次。目前高风险和极高风险批次共占{(risk_summary.get('极高风险', 0) + risk_summary.get('高风险', 0)) / sum(risk_summary.values()) * 100:.1f}%，需要重点关注。
            ...
            with cols[1]:
                ...
                # 添加图表解读
                add_chart_explanation(f"""
                <b>图表解读：</b> 条形图展示了不同风险等级批次的总库存价值。高风险和极高风险批次共占用资金{format_yuan(high_risk_value)}，占总库存价值的{high_risk_value / batch_analysis['批次价值'].sum() * 100:.1f}%。
                ...
                for i, product in enumerate(high_risk_products.itertuples()):
                    ...
                    with cols[i % 2]:
                        ...
                        <div class="metric-card" style="border-left: 5px solid {risk_color};">
                            <p class="card-header">{product.物料} | {product.风险程度}</p>
        ...
        with tabs[1]:  # 库龄与清库分析
            ...
            if not age_data.empty:
                ...
                # 添加图表解读
                add_chart_explanation(f"""
                <b>图表解读：</b> 此图表展示了库龄最长的批次，红线表示90天高风险阈值，橙线表示60天中风险阈值，绿线表示30天低风险阈值。
                批次库龄越长，资金占用成本越高，过期风险越大。目前有{sum(age_data['库龄'] > 90)}个批次库龄超过90天，需要紧急处理。
                ...
                add_chart_explanation(f"""
                                ...
                                另有{over_90_count}个批次预计清库时间超过90天。预计清库天数远大于当前库龄的批次面临严重的积压风险。
        ...
        with tabs[2]:  # 预测偏差分析
            ...
            if not bias_data.empty:
                ...
                add_chart_explanation(f"""
                                ...
                                有{positive_bias}个批次预测显著过高(>30%)，{negative_bias}个批次预测显著过低(<-30%)。
                ...
                # 添加图表解读
                high_right = sum((abs(bias_data['预测偏差数值']) < 0.15) & (bias_data['预计清库天数'] < 60))

                add_chart_explanation(f"""
                                <b>图表解读：</b> 此散点图展示了预测准确度与清库天数的关系。右上和左上象限代表预测偏差大且清库困难的产品，是最需要关注的区域。
                                有{high_right}个批次位于中间区域，表示预测较准确且清库压力不大。点的大小代表批次价值，颜色代表风险等级。
                                <b>行动建议：</b> 重点关注位于右上象限的高价值产品，它们存在预测过高和清库困难的双重问题；对位于左上象限的产品，需检查销售渠道是否存在问题；
                                位于右下和左下象限的产品，虽有预测偏差但清库压力不大，优先级可降低。
                                """)
            else:
        ...
        with tabs[3]:  # 责任分析与跟踪
            ...
            # 责任区域分布图
            st.markdown('<div class="sub-header">责任区域分析</div>', unsafe_allow_html=True)

            cols = st.columns(2)
            with cols[0]:
                region_fig = create_responsibility_region_chart(batch_analysis)
                st.plotly_chart(region_fig, use_container_width=True)

                # 添加图表解读
                add_chart_explanation("""
                                <b>图表解读：</b> 此饼图展示了库存批次的责任区域分布。占比最大的区域承担了最多的库存批次责任，应重点关注其库存管理水平。
                                <b>行动建议：</b> 对责任批次较多的区域进行专项库存管理培训；定期组织区域间的库存管理经验分享；建立区域库存责任追踪机制。
                                """)

            with cols[1]:
                person_fig = create_responsibility_person_chart(batch_analysis)
                st.plotly_chart(person_fig, use_container_width=True)

                # 添加图表解读
                add_chart_explanation("""
                                <b>图表解读：</b> 此条形图展示了责任人的平均风险得分排名。得分越高的责任人对应的库存风险越大，需要重点关注其库存管理行为。
                                <b>行动建议：</b> 对风险得分高的责任人进行针对性辅导；建立销售预测与库存责任挂钩的考核机制；优化高风险责任人的库存分配策略。
                                """)

            # 责任履行分析
            st.markdown('<div class="sub-header">预测履行率分析</div>', unsafe_allow_html=True)

            fulfillment_fig = create_fulfillment_rate_chart(batch_analysis)
            st.plotly_chart(fulfillment_fig, use_container_width=True)

            # 添加图表解读
            add_chart_explanation("""
                            <b>图表解读：</b> 此热力图展示了各责任人与区域的预测履行率。颜色越绿表示履行率越高（实际销售接近预测量），颜色越红表示履行率越低（实际销售远低于预测量）。
                            <b>行动建议：</b> 关注履行率低的责任人-区域组合，分析预测失误原因；对表现良好的责任人总结经验并推广；考虑调整预测方法或责任分配机制。
                            """)

            # 高风险批次责任详情
            st.markdown('<div class="sub-header">高风险批次责任分析</div>', unsafe_allow_html=True)

            # 获取高风险产品数据
            high_risk_data = batch_analysis[batch_analysis['风险程度'].isin(['极高风险', '高风险'])]

            if not high_risk_data.empty:
                # 显示高风险批次责任详情表格
                st.dataframe(
                    high_risk_data[
                        ['物料', '批次日期', '批次库存', '批次价值', '库龄', '预计清库天数', '责任人', '责任区域',
                         '责任分析摘要']],
                    hide_index=True,
                ...
                # 添加表格解读
                add_chart_explanation(f"""
                                <b>表格解读：</b> 此表格列出了所有高风险批次的责任分析详情。可以看到每个批次的责任归属以及责任判定的原因摘要。
                                <b>行动建议：</b> 组织责任人定期复盘批次积压原因；建立责任落实与积压消除的任务跟踪机制；优化预测与库存管理流程，降低整体积压风险。
                                """)
            else:
                st.info("没有高风险批次责任数据，请调整筛选条件。")
