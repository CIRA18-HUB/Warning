...
# 定义文件路径（使用提供的绝对路径）
INVENTORY_FILE = r"C:\Users\何晴雅\Desktop\含批次库存0221(2).xlsx"
SHIPPING_FILE = r"C:\Users\何晴雅\Desktop\2409~250224出货数据.xlsx"
FORECAST_FILE = r"C:\Users\何晴雅\Desktop\2409~2502人工预测.xlsx"
PRICE_FILE = r"C:\Users\何晴雅\Desktop\单价.xlsx"
...
def login_ui():
    """显示登录界面"""
    st.markdown(
        '<div style="font-size: 1.5rem; color: #1f3867; text-align: center; margin-bottom: 1rem;">SAL库存异常与预警 | 登录</div>',
        unsafe_allow_html=True)
...
def check_authentication():
    ...
    # 在开发环境中，可以选择跳过认证
    if st.sidebar.checkbox("跳过认证（开发模式）", value=False, key="skip_auth"):
        st.session_state.authenticated = True
        return True
...
class DataLoader:
    ...
    def load_shipping_data(self):
        ...
        try:
            ...
            # 确保列名符合要求
            expected_columns = ['订单日期', '所属区域', '申请人', '产品代码', '数量']
            if len(self.shipping_data.columns) >= len(expected_columns):
                self.shipping_data.columns = expected_columns + list(self.shipping_data.columns[len(expected_columns):])
            else:
                # 如果列数不足，则添加所需的列
                for i, col in enumerate(expected_columns):
                    if i < len(self.shipping_data.columns):
                        self.shipping_data.columns.values[i] = col
                    else:
                        self.shipping_data[col] = None
    ...
    def load_forecast_data(self):
        ...
        try:
            ...
            # 调整预测数据的格式
            expected_columns = ['所属大区', '销售员', '所属年月', '产品代码', '预计销售量']
            ...
            if len(self.forecast_data.columns) == 1:
                ...
                # 确保列名符合要求
                if len(self.forecast_data.columns) >= len(expected_columns):
                    self.forecast_data.columns = expected_columns + list(
                        self.forecast_data.columns[len(expected_columns):])
                else:
                    # 如果列数不足，则添加所需的列
                    for i, col in enumerate(expected_columns):
                        if i < len(self.forecast_data.columns):
                            self.forecast_data.columns.values[i] = col
                        else:
                            self.forecast_data[col] = None
    ...
    def load_price_data(self):
        ...
        try:
            if os.path.exists(self.price_file):
                ...
                st.warning(f"找不到单价文件: {self.price_file}，使用默认单价数据")
...
class AnalysisEngine:
    ...
    def analyze_data(_self, filtered_data=None):
        ...
        for _, batch in batch_data_to_analyze.iterrows():
            ...
            # 获取产品单价并计算批次价值
            unit_price = _self.price_data.get(product_code, 50.0)  # 默认单价50元
...
def main():
    ...
    # 设置页面标题
    st.markdown('<div class="main-header">SAL库存异常与预警</div>', unsafe_allow_html=True)
    ...
    with st.sidebar:
        ...
        # 显示文件路径信息
        st.write("库存数据文件:", os.path.basename(INVENTORY_FILE))
        st.write("出货数据文件:", os.path.basename(SHIPPING_FILE))
        st.write("预测数据文件:", os.path.basename(FORECAST_FILE))
        st.write("单价数据文件:", os.path.basename(PRICE_FILE))
        ...
        try:
            ...
            if not batch_analysis.empty:
                st.markdown("### 📥 导出数据")
                st.markdown(download_excel(batch_analysis, "SAL库存异常与预警分析结果.xlsx"), unsafe_allow_html=True)

        except Exception as e:
            st.error(f"数据加载或分析错误: {str(e)}")
            st.write("请检查文件路径是否正确，或尝试刷新应用。")
            return
    ...
    with tabs[0]:  # 风险预警概览
        ...
        with cols[0]:
            ...
            # 添加图表解读
            risk_percentage = (risk_summary.get('极高风险', 0) + risk_summary.get('高风险', 0)) / sum(
                risk_summary.values()) * 100 if sum(risk_summary.values()) > 0 else 0
            add_chart_explanation(f"""
            <b>图表解读：</b> 饼图展示了所有库存批次的风险分布情况，红色和橙色部分表示需要优先关注的高风险批次。目前高风险和极高风险批次共占{risk_percentage:.1f}%，需要重点关注。
        ...
        with cols[1]:
            ...
            # 添加图表解读
            value_percentage = high_risk_value / batch_analysis['批次价值'].sum() * 100 if batch_analysis[
                                                                                               '批次价值'].sum() > 0 else 0
            add_chart_explanation(f"""
            <b>图表解读：</b> 条形图展示了不同风险等级批次的总库存价值。高风险和极高风险批次共占用资金{format_yuan(high_risk_value)}，占总库存价值的{value_percentage:.1f}%。
        ...
        high_risk_products = analysis_engine.get_high_risk_products(limit=6)
        if not high_risk_products.empty:
            ...
            for i, product in enumerate(high_risk_products.itertuples()):
                ...
                with cols[i % 3]:
                    ...
                    clearance_text = '无法预测' if product.预计清库天数 == float(
                        'inf') else f'{int(product.预计清库天数)}天'
                    st.markdown(f"""
                    <div class="metric-card" style="border-left: 5px solid {risk_color};">
                        ...
                        <p class="card-text">库龄: {product.库龄}天 | 清库预测: {clearance_text}</p>
                        <p class="card-text">建议: {product.建议措施}</p>
                    </div>
                    """, unsafe_allow_html=True)
        else:
            st.info("没有高风险产品，或者已经被筛选条件过滤。")
    ...
    with tabs[1]:  # 库龄与清库分析
        ...
        if not age_data.empty:
            ...
            # 添加图表解读
            over_90_count = sum(age_data['库龄'] > 90)
            add_chart_explanation(f"""
            <b>图表解读：</b> 此图表展示了库龄最长的批次，红线表示90天高风险阈值，橙线表示60天中风险阈值，绿线表示30天低风险阈值。
            批次库龄越长，资金占用成本越高，过期风险越大。目前有{over_90_count}个批次库龄超过90天，需要紧急处理。
