...
def analyze_responsibility_collaborative(shipping_data, forecast_data, sales_person_region_mapping, product_code,
    ...
    if not product_forecasts.empty:
        ...
        for person in person_forecast_totals.index:
            # 确保使用正确的列名 '求和项:数量（箱）'
            person_actual_sales = product_sales[product_sales['申请人'] == person]['求和项:数量（箱）'].sum() if not product_sales.empty else 0
...
def assess_batch_risk(batch_data, shipping_data, forecast_data, sales_person_region_mapping):
    ...
    batch_data = batch_data.copy()  # 创建副本避免警告

    # 确保存在物料列和产品代码列
    if '物料' in batch_data.columns and '产品代码' not in batch_data.columns:
        batch_data['产品代码'] = batch_data['物料']
    elif '产品代码' in batch_data.columns and '物料' not in batch_data.columns:
        batch_data['物料'] = batch_data['产品代码']
    elif '物料' not in batch_data.columns and '产品代码' not in batch_data.columns:
        st.error("批次数据中缺少必要的列: 物料 或 产品代码")
        return pd.DataFrame()
    ...
    for _, batch in batch_data.iterrows():
        try:
            ...
            risk_assessment.append({
                '物料': product_code,
                '产品代码': product_code,  # 添加产品代码列，以兼容其他函数
                '描述': batch['描述'],
...
# 显示风险分布饼图
# 修改 display_risk_distribution_chart 函数
# 显示风险分布饼图
def display_risk_distribution_chart(risk_assessment, key_suffix=""):
    """
    显示批次风险分布饼图
    """
    ...
    # 使用唯一的key
    st.plotly_chart(fig, use_container_width=True, key=f"risk_dist_chart_{key_suffix}")
...
# 显示高风险批次库龄分布
# 显示高风险批次库龄分布
def display_high_risk_age_chart(risk_assessment, key_suffix=""):
    """
    显示高风险批次的库龄分布
    """
    # 筛选高风险和极高风险批次
    ...
    # 使用唯一的key
    st.plotly_chart(fig, use_container_width=True, key=f"high_risk_age_chart_{key_suffix}")
...
# 显示清库预测图
# 显示清库预测图
def display_clearance_forecast_chart(risk_assessment, key_suffix=""):
    """
    显示批次清库预测时间图表
    """
    if risk_assessment.empty:
    ...
    # 使用唯一的key
    st.plotly_chart(fig, use_container_width=True, key=f"clearance_forecast_chart_{key_suffix}")
...
# 显示责任分析图
# 显示责任分析图
def display_responsibility_analysis(risk_assessment, key_suffix=""):
    """
    显示责任人和区域的批次数量分析
    """
    if risk_assessment.empty:
    ...
    # 使用唯一的key
    st.plotly_chart(fig, use_container_width=True, key=f"resp_person_chart_{key_suffix}")
    ...
    if not region_counts.empty:
        ...
        # 使用唯一的key
        st.plotly_chart(fig, use_container_width=True, key=f"resp_region_chart_{key_suffix}")
...
# 风险详情列表组件
# 风险详情列表组件
def display_risk_detail_list(risk_assessment, filter_option=None, key_suffix=""):
    """
    显示风险详情列表，支持筛选
    """
    if risk_assessment.empty:
    ...
    # 为每个批次创建详情卡片
    for i, (_, batch) in enumerate(filtered_data.head(10).iterrows()):
        ...
        # 创建卡片，使用唯一的key作为HTML元素ID
        card_id = f"risk_card_{key_suffix}_{i}"
        st.markdown(f"""
        <div id="{card_id}" class="risk-card {risk_class}">
    ...
    if len(filtered_data) > 10:
        ...
        # 使用唯一的key
        button_key = f"show_all_{filter_option}_{key_suffix}"
        if st.button(f"查看全部{len(filtered_data)}个批次", key=button_key):
            # 显示完整表格
...
# 产品趋势折线图
def display_product_trend_chart(product_growth):
    """显示产品销量增长率的历史趋势折线图"""
    if 'all_growth' not in product_growth or product_growth['all_growth'].empty:
        st.warning("没有足够的历史数据来生成产品趋势折线图")
        return

    # 获取所有月份的增长数据
    all_growth_data = product_growth['all_growth'].copy()

    # 创建年月字段用于排序和显示
    all_growth_data['年月'] = all_growth_data.apply(lambda row: f"{row['年']}-{str(row['月']).zfill(2)}", axis=1)

    # 按产品代码和年月排序
    all_growth_data = all_growth_data.sort_values(['产品代码', '年', '月'])

    # 获取唯一的产品列表
    unique_products = all_growth_data['产品代码'].unique()

    # 为产品名称添加格式化
    all_growth_data['产品显示'] = all_growth_data['产品代码'].apply(
    ...
    # 添加每个产品的趋势线（限制显示前10个产品，避免图表过于拥挤）
    for product in unique_products[:10]:
        product_data = all_growth_data[all_growth_data['产品代码'] == product]
        if product_data.empty:
            continue

        product_name = product_data['产品显示'].iloc[0]

        fig.add_trace(go.Scatter(
            x=product_data['年月'],
            y=product_data['销量增长率'],
            mode='lines+markers',
            name=product_name,
            line=dict(width=2),
            marker=dict(size=8),
            hovertemplate='<b>%{text}</b><br>月份: %{x}<br>增长率: %{y:.1f}%<extra></extra>',
            text=[product_name] * len(product_data)
        ))

    # 添加零线
    min_month = all_growth_data['年月'].min() if not all_growth_data.empty else ""
    max_month = all_growth_data['年月'].max() if not all_growth_data.empty else ""
    ...
    # 添加解释
    trend_explanation = """
    <b>图表解读：</b> 此折线图展示了产品销量增长率的历史趋势。每条线代表一个产品，可以观察其销量变化趋势。
    图表显示了全部历史月份的增长率数据，零线(虚线)以上表示增长，零线以下表示下降。
    可以通过此图发现产品的季节性模式和长期趋势，帮助制定更精准的备货策略。
    """
...
def load_product_info(file_path=None):
    ...
    try:
        ...
        if file_path is None or not os.path.exists(file_path):
            st.error(f"产品信息文件不存在: {file_path}")
        ...
        if missing_columns:
            st.error(f"产品信息文件缺少必要的列: {', '.join(missing_columns)}")
            return pd.DataFrame()  # 返回空DataFrame
...
def load_batch_inventory(file_path=None):
    ...
    try:
        # 检查文件路径是否有效
        if file_path is None or not os.path.exists(file_path):
            st.error(f"批次库存文件不存在: {file_path}")
            return pd.DataFrame()  # 返回空DataFrame
        ...
        for i, row in inventory_raw.iterrows():
            ...
            elif i > 0 and pd.isna(row.iloc[0]) and pd.notna(row.iloc[7]):
                # 这是批次行，确保是有效的批次数据行（至少库位有值）
                try:
                    # 提取需要的列
                    library = row.iloc[7]  # 库位
                    prod_date = row.iloc[8]  # 生产日期
                    batch_no = row.iloc[9]  # 生产批号
                    quantity = row.iloc[10]  # 数量
                    ...
                    print(f"处理第{i + 1}行批次数据时出错: {str(e)}")
                    continue
        ...
        if batch_with_product:
            ...
            # 转换日期列 - 增加更强的错误处理和格式兼容性
            try:
                # 尝试转换日期，处理多种可能的格式
                batch_data['生产日期'] = pd.to_datetime(batch_data['生产日期'], errors='coerce')

                # 检查并填充无效日期
                if batch_data['生产日期'].isna().any():
                    st.warning(f"发现{batch_data['生产日期'].isna().sum()}个无效日期，已替换为当前日期")
                    batch_data.loc[batch_data['生产日期'].isna(), '生产日期'] = pd.Timestamp.now()
            except Exception as e:
                st.warning(f"转换日期时出错: {str(e)}。将保留原始日期格式。")

            # 确保数值列为数值类型
            try:
                batch_data['数量'] = pd.to_numeric(batch_data['数量'], errors='coerce').fillna(0)
            except Exception as e:
                st.warning(f"转换数量列时出错: {str(e)}")

            return batch_data
        else:
            st.error("未能提取到有效的批次数据")
            return pd.DataFrame()

    except Exception as e:
        st.error(f"加载批次库存数据时出错: {str(e)}")
        return pd.DataFrame()  # 返回空DataFrame


def create_product_names_map(product_info_df):
    """创建产品代码到产品名称的映射字典"""
    if product_info_df is None or product_info_df.empty:
        return {}

    product_names_map = {}
    for _, row in product_info_df.iterrows():
        if '产品代码' in product_info_df.columns and '产品名称' in product_info_df.columns:
            product_code = row['产品代码']
            product_name = row['产品名称']
            if pd.notna(product_code) and pd.notna(product_name):
                product_names_map[str(product_code)] = str(product_name)

    return product_names_map

# 新增函数：全局筛选组件
def create_global_filters(all_months, all_regions, valid_last_three_months, key_prefix=""):
    """创建全局筛选组件"""
...
# 新增函数：整合趋势图表
def display_combined_trend_chart(trend_data, title="销售与预测趋势分析"):
    """整合的销售与预测趋势图表"""
    fig = go.Figure()
...
# 新增函数：整合库存风险分析
def display_combined_risk_analysis(risk_assessment, key_suffix=""):
    """整合的库存风险分析，合并多个风险图表"""

    if risk_assessment.empty:
        st.warning("未发现批次数据，无法进行风险分析")
        return

    # 创建选项卡，将多个风险图表整合在一起
    risk_tabs = st.tabs(["风险分布", "高风险批次", "清库预测", "责任分析"])

    with risk_tabs[0]:
        display_risk_distribution_chart(risk_assessment, key_suffix=f"{key_suffix}_dist")

    with risk_tabs[1]:
        display_high_risk_age_chart(risk_assessment, key_suffix=f"{key_suffix}_age")

    with risk_tabs[2]:
        display_clearance_forecast_chart(risk_assessment, key_suffix=f"{key_suffix}_forecast")

    with risk_tabs[3]:
        display_responsibility_analysis(risk_assessment, key_suffix=f"{key_suffix}_resp")


# 新增函数：总览标签页
def display_overview_tab(processed_data, selected_months, selected_regions, product_info, risk_assessment):
    """总览与历史标签页"""
    # 筛选数据
    ...
    # 提供视图选择器
    view_option = st.radio("选择分析视图", ["销售预测趋势", "区域销售分析"], horizontal=True)

    if view_option == "销售预测趋势":
        st.markdown('<div class="sub-header">📊 销售与预测历史趋势</div>', unsafe_allow_html=True)

        # 准备历史趋势数据
        monthly_trend = filtered_monthly.groupby(['所属年月']).agg({
            '求和项:数量（箱）': 'sum',
            '预计销售量': 'sum'
        }).reset_index()

        # 使用整合的趋势图表
        display_combined_trend_chart(monthly_trend)
    else:
...
# 新增函数：预测分析标签页
def display_forecast_analysis_tab(processed_data, selected_months, selected_regions, product_info):
    """预测差异分析标签页"""
    # 筛选数据
    diff_filtered_monthly = filter_data(processed_data['merged_monthly'], selected_months, selected_regions)
    diff_filtered_salesperson = filter_data(processed_data['merged_by_salesperson'], selected_months, selected_regions)
    ...
    if analysis_dimension == '产品':
        diff_data = diff_filtered_monthly.groupby(['产品代码']).agg({
            '求和项:数量（箱）': 'sum',
            '预计销售量': 'sum',
        }).reset_index()

        # 计算差异和差异率
        diff_data['数量差异'] = diff_data['求和项:数量（箱）'] - diff_data['预计销售量']
        diff_data['数量差异率'] = diff_data['数量差异'] / diff_data['求和项:数量（箱）'] * 100

        # 处理产品名称显示
        diff_data['产品显示'] = diff_data['产品代码'].apply(
            lambda x: format_product_code(x, product_info, include_name=True)
        )

        # 按差异率绝对值降序排序
        diff_data = diff_data.sort_values('数量差异率', key=abs, ascending=False)
        ...
        for i, row in diff_data.iterrows():
            fig_diff.add_annotation(
                y=row['产品显示'],
                x=max(row['求和项:数量（箱）'], row['预计销售量']) * 1.05,
                text=f"{row['数量差异率']:.1f}%",
                showarrow=False,
                font=dict(
                    color="green" if row['数量差异率'] >= 0 else "red"
                )
            )

        # 更新布局
        fig_diff.update_layout(
            title="预测与实际销售差异分析 (按产品)",
            barmode='group',
            xaxis=dict(
                title="销售量 (箱)",
                tickformat=",",
            ...
            legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1),
            plot_bgcolor='white',
            height=max(500, len(diff_data) * 25)  # 动态调整高度
        )

        st.plotly_chart(fig_diff, use_container_width=True)

    else:  # 销售员维度
        diff_data = diff_filtered_salesperson.groupby(['销售员']).agg({
            '求和项:数量（箱）': 'sum',
            '预计销售量': 'sum'
        }).reset_index()
    ...
    <div class="chart-explanation">
        <b>图表解读：</b> 此图展示了预测与实际销售量的差异。正差异率(绿色)表示实际销售量高于预测(低估)，
        负差异率(红色)表示实际销售量低于预测(高估)。较大的差异表明预测精度有待提升。
    </div>
    """, unsafe_allow_html=True)


# 新增函数：产品趋势标签页
def display_product_trend_tab(actual_data, product_info, risk_assessment, selected_months, selected_regions):
    """产品趋势标签页"""
    ...
    if 'latest_growth' in product_growth and not product_growth['latest_growth'].empty:
        ...
        # 提供视图选项
        view_option = st.radio("选择分析视图", ["增长率趋势", "备货建议"], horizontal=True)

        if view_option == "增长率趋势":
            st.markdown('<div class="sub-header">📊 产品销量趋势分析</div>', unsafe_allow_html=True)
            display_product_trend_chart(product_growth)
        else:
        ...
        if not risk_assessment.empty:
            ...
            if 'latest_growth' in product_growth and not product_growth['latest_growth'].empty:
                ...
                # 合并风险评估数据 - 修复列名问题
                risk_data = risk_assessment.copy()
                if '物料' in risk_data.columns and '产品代码' not in risk_data.columns:
                    risk_data['产品代码'] = risk_data['物料']

                combined_data = pd.merge(
                    risk_data[['产品代码', '批次库存', '库龄', '日均出货', '预计清库天数', '风险程度']],
                    growth_data,
                    on='产品代码',
                    how='inner'
                )
                ...
                if not combined_data.empty:
                    ...
                    if not declining_products.empty:
                        ...
                        st.markdown("""
                        <div class="chart-explanation">
                            <b>图表解读：</b> 此图展示了销量下降产品的库存清理预测。红色柱形表示销量增长率(负值表示下降)，
                            蓝色线条表示预计清库天数。红色虚线(90天)为风险阈值，超过此阈值的产品应优先处理。
                        </div>
                        """, unsafe_allow_html=True)
                    else:
        ...
        st.warning("没有足够的历史数据来计算产品增长率。需要至少两个月的销售数据才能计算增长率。")


# 新增函数：风险管理标签页
def display_risk_management_tab(risk_assessment):
    """库存风险管理标签页"""
    ...
    if not risk_assessment.empty:
        # 使用整合的风险分析组件
        display_combined_risk_analysis(risk_assessment, key_suffix="tab4")

        # 风险批次详情列表
        st.markdown('<div class="sub-header">📋 批次风险详情</div>', unsafe_allow_html=True)

        # 显示风险批次详情列表 - 使用原有函数
        display_risk_detail_list(risk_assessment, selected_risk, key_suffix="tab4")
    else:
...
def load_actual_data(file_path=None):
    ...
    try:
        # 检查文件路径是否有效
        if file_path is None or not os.path.exists(file_path):
            st.error(f"实际销售数据文件不存在: {file_path}")
            return pd.DataFrame()  # 返回空DataFrame
        ...
        if missing_columns:
            st.error(f"实际销售数据文件缺少必要的列: {', '.join(missing_columns)}")
            return pd.DataFrame()  # 返回空DataFrame
...
def load_forecast_data(file_path=None):
    ...
    try:
        # 检查文件路径是否有效
        if file_path is None or not os.path.exists(file_path):
            st.error(f"预测数据文件不存在: {file_path}")
            return pd.DataFrame()  # 返回空DataFrame
        ...
        if missing_columns:
            st.error(f"预测数据文件缺少必要的列: {', '.join(missing_columns)}")
            return pd.DataFrame()  # 返回空DataFrame
        ...
        try:
            ...
            st.warning(f"转换所属年月时出错: {str(e)}。尝试替代方法。")
            # 尝试从列中提取年月格式
            if isinstance(df['所属年月'].iloc[0], str):
                # 如果已经是字符串，尝试统一格式
                df['所属年月'] = df['所属年月'].str.replace(r'(\d{4})(\d{2})', r'\1-\2', regex=True)
            else:
                st.error("无法处理所属年月格式")
                return pd.DataFrame()
...
def load_price_data(file_path=None):
    ...
    try:
        if file_path and os.path.exists(file_path):
            ...
            st.warning(f"单价数据文件不存在: {file_path}，使用默认单价")
...
def display_recommendations_table(latest_growth, product_info):
    ...
    for i, row in enumerate(display_data.itertuples()):
        # 确保所需属性存在且有效
        growth_rate = getattr(row, '销量增长率', 0)
        if not pd.isna(growth_rate):
            fig.add_annotation(
                x=growth_rate,
                y=row.产品显示,
                text=f"{growth_rate:.1f}% {getattr(row, '建议图标', '') if hasattr(row, '建议图标') and pd.notna(getattr(row, '建议图标', '')) else ''}",
                showarrow=False,
                xshift=10 if growth_rate >= 0 else -10,
                align="left" if growth_rate >= 0 else "right",
                font=dict(
                    color="#43A047" if growth_rate >= 0 else "#E53935",
                    size=10
                )
            )
...
if use_default_files:
    ...
    # 创建产品名称映射 - 新增的代码行
    product_names_map = create_product_names_map(product_info)
...
if not batch_data.empty:
    if '物料' in batch_data.columns and '产品代码' not in batch_data.columns:
        batch_data = batch_data.rename(columns={'物料': '产品代码'})
    elif '产品代码' in batch_data.columns and '物料' not in batch_data.columns:
        batch_data['物料'] = batch_data['产品代码']
...
# 创建标签页 - 精简为4个标签页
tabs = st.tabs(["📊 总览", "🔍 预测分析", "📈 产品与库存", "⚠️ 风险管理"])

with tabs[0]:  # 总览标签页
    display_overview_tab(processed_data, selected_months, selected_regions, product_info, risk_assessment)

with tabs[1]:  # 预测分析标签页
    display_forecast_analysis_tab(processed_data, selected_months, selected_regions, product_info)

with tabs[2]:  # 产品与库存标签页
    display_product_trend_tab(actual_data, product_info, risk_assessment, selected_months, selected_regions)

with tabs[3]:  # 风险管理标签页
    display_risk_management_tab(risk_assessment)
...
with tabs[0]:  # 总览与历史标签页
    ...
    if not selected_months or not selected_regions:
        ...
        if not risk_assessment.empty:
            ...
            # 显示风险分布饼图
            display_risk_distribution_chart(risk_assessment, key_suffix="tab0")
    ...
    if not trend_selected_months or not trend_selected_regions:
        ...
        if 'latest_growth' in product_growth and not product_growth['latest_growth'].empty:
            ...
            def display_product_trend_chart(product_growth):
                ...
                # 创建年月字段用于排序和显示
                all_growth_data['年月'] = all_growth_data.apply(lambda row: f"{row['年']}-{str(row['月']).zfill(2)}",
                                                                axis=1)
                ...
                for product in unique_products[:10]:
                    ...
                    fig.add_trace(go.Scatter(
                        ...
                        line=dict(width=2),
                        marker=dict(size=8),
                        hovertemplate='<b>%{text}</b><br>月份: %{x}<br>增长率: %{y:.1f}%<br>当月销量: %{customdata}箱<extra></extra>',
                        text=[product_name] * len(product_data),
                        customdata=product_data['当月销量'].fillna(0).astype(int)
                    ))
                ...
                if 'latest_growth' in product_growth and not product_growth['latest_growth'].empty:
                    ...
                    if not combined_data.empty:
                        ...
                        if not declining_products.empty:
                            ...
                            st.plotly_chart(fig, use_container_width=True, key="declining_products_chart")
...
with tabs[4]:  # 库存风险管理标签页
    ...
    if not risk_assessment.empty:
        ...
        # 显示风险分布饼图
        display_risk_distribution_chart(risk_assessment, key_suffix="tab4")

        # 高风险批次库龄分析
        st.markdown('<div class="sub-header">📊 批次库龄分析</div>', unsafe_allow_html=True)

        # 显示高风险批次库龄分布图
        display_high_risk_age_chart(risk_assessment, key_suffix="tab4")

        # 清库预测分析
        st.markdown('<div class="sub-header">📊 清库预测分析</div>', unsafe_allow_html=True)

        # 显示清库预测图
        display_clearance_forecast_chart(risk_assessment, key_suffix="tab4")

        # 责任分析
        st.markdown('<div class="sub-header">📊 责任分析</div>', unsafe_allow_html=True)

        # 显示责任分析图
        display_responsibility_analysis(risk_assessment, key_suffix="tab4")
