...
def login_ui():
    """显示登录界面"""
    st.markdown('<div style="font-size: 1.5rem; color: #1f3867; text-align: center; margin-bottom: 1rem;">SAL异常与预警 | 登录</div>', unsafe_allow_html=True)
...
class AnalysisEngine:
    ...
    def analyze_responsibility_collaborative(self, product_code, batch_date, batch_qty=0):
        ...
        if forecast_responsibility_details and batch_qty > 0:
            ...
            # 如果没有预测数据或批次数量为0
            max_person = max(person_scores.items(), key=lambda x: x[1])[0] if person_scores else default_mapping["person"]
    ...
    def generate_responsibility_summary(self, responsibility_analysis):
        ...
        if responsible_person in forecast_responsibility:
            ...
            if forecast_qty > 0:
                main_person_reasons.append(f"预测{forecast_qty:.0f}件但仅销售{actual_sales:.0f}件(履行率{fulfillment:.0f}%)")
    ...
    def filter_data(self, product_filter=None, region_filter=None, person_filter=None, risk_filter=None):
        ...
        if region_filter and isinstance(region_filter, list) and len(region_filter) > 0:
            ...
            region_products = self.shipping_data[self.shipping_data['所属区域'].isin(region_filter)]['产品代码'].unique()
        ...
        filtered_shipping_data = self.shipping_data[shipping_filter] if isinstance(shipping_filter, pd.Series) else self.shipping_data
...
def main():
    ...
    with st.sidebar:
        ...
        try:
            ...
            # 责任人筛选器
            all_persons = sorted([p for p in data_loader.shipping_data['申请人'].unique() if p != data_loader.default_person])
